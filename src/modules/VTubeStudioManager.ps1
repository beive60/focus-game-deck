    ...
    [bool] Authenticate() {
        try {
            ...
            if (-not $authToken) {
                ...
                # DEBUG: Log the raw response (without color parameters per architecture guidelines)
                Write-Verbose "[DEBUG] VTubeStudioManager: Token response received: $($tokenResponse | ConvertTo-Json -Depth 5 -Compress)"
                ...
                if ($tokenResponse.messageType -ne "AuthenticationTokenResponse") {
                    Write-Verbose "[DEBUG] VTubeStudioManager: Unexpected messageType: $($tokenResponse.messageType)"
