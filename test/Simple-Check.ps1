...
if ($timeDiff.TotalMinutes -lt 10) {
    Write-Host "[OK] File was recently updated" -ForegroundColor Green
} else {
...
if ($config.obs.websocket.host -eq "testhost") {
    Write-Host "[OK] OBS Host changed to testhost" -ForegroundColor Green
...
if ($config.obs.websocket.port -eq 4456) {
    Write-Host "[OK] OBS Port changed to 4456" -ForegroundColor Green
...
if ($config.language -eq "ja") {
    Write-Host "[OK] Language changed to Japanese" -ForegroundColor Green
...
if ($testGames) {
    Write-Host "[OK] Test game(s) found:" -ForegroundColor Green
