...
function Test-Prerequisites {
    ...
    if (-not (Test-Path $mainWindowPath) -and -not $Global:Xaml_MainWindow) {
        $issues += "Required file not found and no embedded XAML available: $mainWindowPath"
    }

    # Check fallback language file (required for localization)
    $enJsonPath = Join-Path -Path $script:appRoot -ChildPath "localization/en.json"
    if (-not (Test-Path $enJsonPath)) {
        $issues += "Required fallback language file not found: $enJsonPath (English localization is mandatory for fallback)"
    }

    # Note: config.json is NOT a required file at startup
    # It will be generated by the GUI if it does not exist
...
function Initialize-ConfigEditor {
    ...
    try {
        ...
        try {
            ...
            # Phase 3: Set Window reference in state manager for title bar updates
            $stateManager.Window = $uiManager.Window

            # Phase 3: Set localized base window title
            $windowTitleKey = "windowTitle"
            $localizedTitle = $localization.GetMessage($windowTitleKey, @())
            if ($localizedTitle) {
                $stateManager.BaseWindowTitle = $localizedTitle
            }
